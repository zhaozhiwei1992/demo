...
//        });
    ...
    public static void main(String[] args) throws IOException, InterruptedException, KeeperException {
        ...
        Stat exists = zooKeeper.exists("/tt/tt2", true);
//        Stat exists = null;
        if(exists == null){
            zooKeeper.create("/tt/tt2", "xxx".getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
            //这里又写了一次下面语句才出发事件, 也可以将下面的语句写入到watcher中， 触发后创建事件后就执行下面语句
            //同样达到相同的效果
//            zooKeeper.exists("/tt/tt2", true);
            zooKeeper.setData("/tt/tt2", "xx00".getBytes(), -1);
//            zooKeeper.exists("/tt/tt2", true);
            zooKeeper.delete("/tt/tt2", -1);
        }else{
            zooKeeper.delete("/tt/tt2", -1);
        }
    }
